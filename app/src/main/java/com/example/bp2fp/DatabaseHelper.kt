...
//            private val COLUMN_JADWAL_DOCOTOR = "jadwalDoctor"
        ...
        private val CREATE_TRANSACTION_TABLE = ("CREATE TABLE " + TABLE_TRANSACTION + "("
                ...
                + COLUMN_ID_DOCT + " INT REFERENCES " + TABLE_DOCTOR + "(" + COLUMN_ID_DOCTOR + ") ON DELETE CASCADE, "
                + COLUMN_NAMA_DOCT + " TEXT REFERENCES " + TABLE_DOCTOR + "(" + COLUMN_NAMA_DOCT + ") ON DELETE CASCADE,"
                + COLUMN_TANGGAL + " DATE)")
    ...
    //    }
        ...
        fun addTransaction(doctorId: Int) {
            ...
            // Check if the doctorId exists in the TABLE_DOCTOR
            val doctorCursor = dbSelect.rawQuery("SELECT * FROM $TABLE_DOCTOR WHERE $COLUMN_ID_DOCTOR = ?", arrayOf(doctorId.toString()))
            if (doctorCursor.count == 0) {
                // Doctor ID not found, show an error message or handle accordingly
            ...
            if (cursorTrans.moveToLast()) {
                lastIdTrans = cursorTrans.getInt(cursorTrans.getColumnIndex(COLUMN_ID_TRANSACTION))
            }
        ...
        data class Transaction(val id: Int, val idDoctor: String, val namaDoctor: String, val tanggal: Int)
        ...
        fun getTransaction(): List<Transaction> {
            ...
            cursor?.use { // Use the cursor in a safe manner
                if (cursor.moveToFirst()) {
                    do {
                        val idIndex = cursor.getColumnIndex(COLUMN_ID_TRANSACTION)
                        val idDoctorIndex = cursor.getColumnIndex(COLUMN_ID_DOCT)
                        val namaDoctorIndex = cursor.getColumnIndex(COLUMN_NAMA_DOCT)
                        val tanggalIndex = cursor.getColumnIndex(COLUMN_TANGGAL)

                        if (idIndex != -1 && idDoctorIndex != -1 && namaDoctorIndex != -1 && tanggalIndex != -1) {
                            val id = cursor.getInt(idIndex)
                            val idDoctor = cursor.getString(idDoctorIndex)
                            val namaDoctor = cursor.getString(namaDoctorIndex)
                            val tanggal = cursor.getInt(tanggalIndex)

                            val transaction = Transaction(id, idDoctor, namaDoctor, tanggal)
